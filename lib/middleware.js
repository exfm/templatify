...
var middleware = module.exports = function(dir, o) {
  ...
  var files = o.files.map(function(f){
    ...
    f = (f.indexOf('/') === 0) ? f : path.join(dir, f).replace(/\\/g, '/');
...
function include(target, content, b) {
  var out = body
    .replace(/\$__filename/g, '"' + target + '"')
    .replace(/\$body/, content);
  b.templatify[target] = {'body': out, 'target': target};
  return out;
}
